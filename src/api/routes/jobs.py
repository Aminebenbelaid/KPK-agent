...
def list_jobs(
    ...
    count_query = _BEST_CTE + f"SELECT COUNT(*) as total FROM best WHERE rn = 1{where_clause}"
    data_query = (
        _BEST_CTE
        + f"SELECT * FROM best WHERE rn = 1{where_clause} ORDER BY {order_clause} LIMIT ? OFFSET ?"
    )
...
def get_job(request: Request, job_id: str):
    ...
    with get_db() as conn:
        row = conn.execute(
            _BEST_CTE + "SELECT * FROM best WHERE rn = 1 AND job_id = ?",
            (sid, job_id),
        ).fetchone()
...
@router.delete("/jobs")
def delete_jobs(request: Request, source: Optional[str] = None):
    """Delete all of this session's jobs, or only those from a given source."""
    sid = request.state.effective_sid
    with get_db() as conn:
        if source:
            cur = conn.execute(
                "DELETE FROM applications WHERE session_id = ? AND json_extract(job_data, '$.source') = ?",
                (sid, source),
            )
        else:
            cur = conn.execute("DELETE FROM applications WHERE session_id = ?", (sid,))
...
def delete_job(request: Request, job_id: str):
    ...
    with get_db() as conn:
        cur = conn.execute(
            "DELETE FROM applications WHERE session_id = ? AND job_id = ?", (sid, job_id)
        )
        deleted = cur.rowcount
    if not deleted:
        raise HTTPException(status_code=404, detail="Job not found")
